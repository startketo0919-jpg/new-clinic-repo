...
echo (Or double-click TVS-Print-Bridge-Silent.vbs to run in background)
echo.

set "TARGET_PS=%TEMP%\tvs-print-bridge.ps1"
if exist "%~dp0tvs-print-bridge.ps1" (
    set "TARGET_PS=%~dp0tvs-print-bridge.ps1"
) else (
    powershell -NoProfile -ExecutionPolicy Bypass -Command "$raw = [System.IO.File]::ReadAllText('%~f0'); $parts = $raw -split '###POWERSHELL_START###\r?\n'; if ($parts.Length -gt 1) { [System.IO.File]::WriteAllText('%TARGET_PS%', $parts[1]) }"
)

powershell -NoProfile -ExecutionPolicy Bypass -File "%TARGET_PS%"

if %ERRORLEVEL% NEQ 0 (
    echo.
    echo Bridge stopped with error code %ERRORLEVEL%.
)
pause
exit /b
...
# Cleanup previous zombie bridge processes
try {
    $myPid = $PID
    Get-CimInstance Win32_Process -ErrorAction SilentlyContinue | Where-Object { 
        $_.CommandLine -like '*tvs-print-bridge*' -and $_.ProcessId -ne $myPid 
    } | ForEach-Object { 
...
try {
    ...
    Write-Warning "Could not start local HTTP listener on $prefix : $($_.Exception.Message)"
}

Write-Output "=================================================="
Write-Output "  Clinic Thermal Printer Bridge (TVS-E RP 3230)  "
Write-Output "=================================================="
Write-Output "Local PC Bridge    : $prefix $(if ($listener.IsListening) {'(Active)'} else {'(Inactive)'})"
...
try {
    while ($true) {
        ...
        if ($contextTask -ne $null) {
            $contextIndex = $tasksToWait.Count
        ...
        if ($tasksToWait.Count -eq 0) {
            ...
            # -------------------------------------------------------------
            if ($completedIndex -ge 0 -and $completedIndex -eq $contextIndex) {
                try {
            ...
            # -------------------------------------------------------------
            elseif ($completedIndex -ge 0 -and $completedIndex -eq $streamIndex) {
